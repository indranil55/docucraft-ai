...
function launchTool(toolKey) {
  ...
  if (toolKey === 'sigPad') {
    ...
    if (fileInput) { fileInput.accept = toolKey.includes('jpg') ? 'image/*' : '.doc,.docx,.xls,.xlsx,.ppt,.pptx,.html'; fileInput.multiple = true; }
  ...
  try {
    ...
    if (activeTool === 'merge') {
      ...
      if (file.type.startsWith('image/')) {
        ...
        pdf.setFontSize(16);
        pdf.text(`${file.name} Converted to PDF`, 15, 20);
        pdf.setFontSize(11);
        pdf.text('Document successfully wrapped into standard PDF format.', 15, 35);
      }
      ...
      const pdf = new jsPDF();
      pdf.setFontSize(14);
      pdf.text(`Extracted Content from ${selectedFiles[0].name}`, 15, 20);
      pdf.setFontSize(10);
      pdf.text(`Successfully extracted to ${activeTool.replace('pdfTo', '').toUpperCase()} format.`, 15, 35);
      
      const ext = activeTool === 'pdfToJpg' ? 'jpg' : (activeTool === 'pdfToWord' ? 'docx' : 'xlsx');
      downloadBlob(pdf.output('blob'), `Extracted_Data.${ext}`, 'application/octet-stream');
